...
class ProvidersStore(MySqlStore):

    def create_provider(self, providerId, providerName):
        provider_entity = Provider

        instance = provider_entity(providerId=providerId, providerName=providerName)
    ...
    def insert(self, providerId, providerName):
        entity = Provider
        instance = entity(providerId=providerId,providerName=providerName)
        with DatabaseSession() as session:
            session.add(instance)
            session.flush()
            session.commit()
    ...
    def delete_provider(self, providerId):
        provider_entity = Provider

        with DatabaseSession() as session:
            data = session.query(provider_entity). \
                filter(provider_entity.providerId == providerId). \
                first()
            if data is None:
                return False
    ...
    def get_providers(self):
        with DatabaseSession() as session:
            data = session.query(Provider).all()
            providers = self.normalize(data)
